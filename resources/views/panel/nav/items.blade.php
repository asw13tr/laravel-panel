...
<div class="row">
     ...
     <div class="col-md-12">
          <h3 class="m5 pull-left">{{ $menu->name }} <small>(Menü Düzenle)</small></h3>
     </div>
     <div class="clearfix"></div>


     <div class="col-md-12">
     @include('panel/nav/item-form')
                         ...
                         <form class="ajaxNavItemForm" data-id="{{ $item->id }}" id="itemForm{{ $item->id }}" action="{{ route('panel.ajax.nav.item.update', ['nav'=>$item]) }}" method="post">
                            ...
                            <td>
                                <select name="target" class="form-control input-sm">
                                    <option value="0"<?php if($item->target==0){ echo ' selected'; }?>>Aynı Pencerede</option>
                                    <option value="1"<?php if($item->target==1){ echo ' selected'; }?>>Yeni Pencerede</option>
                                </select>
                            </td>
                            <td><input type="text" name="css" value="{{ $item->css }}" class="form-control input-sm"></td>
