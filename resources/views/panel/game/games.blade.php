...
<div class="row">
     ...
     <div class="col-md-12">
          <h3 class="m5 pull-left">Oyun Listesi</h3>
          <a href="{{ route('panel.game.create') }}" class="btn btn-primary pull-right">Oyun Ekle</a>
          <div class="clearfix"></div>
          <?php echo listTableNavigation( route('panel.game.games'), url(Request::getRequestUri()) ); ?>
     </div>
     <div class="clearfix"></div>
     <div class="col-md-12">
                    ...
                    <tbody>
                         ...
                         <tr>
                              ...
                              <td data-search="{{ $item->title }}" class="title">
                                  <a href="{{ route('panel.game.edit', ['game'=>$item]) }}">{{ $item->title }}</a><br>
                              ...
                              <td class="row p0">
                                      ...
                                      <a href="{{ route('panel.game.change_comments_permissions', ['game'=>$item]) }}" class="ajaxChangeStatus btn btn-{{ $acClass }} btn-xs"><span class="fa fa-comments"></span></a>
                              </td>
                              <td class="row p0">
                                   <div class="col-xs-4 p0"><a href="{{ route('panel.game.edit', ['game'=>$item]) }}" class="btn btn-primary btn-xs"><span class="fa fa-edit"></span></a></div>
                                   <div class="col-xs-4 p0">

                                        <a href="{{ route('panel.game.change_status', ['game'=>$item]) }}" class="ajaxChangeStatus btn btn-{{ getBool($item->status, "published", "success", "danger") }} btn-xs"><span class="fa fa-eye"></span></a>

                                   </div>
                                   <div class="col-xs-4 p0">
                                        @if( $item->status != "trash" )
                                        <a href="{{ route('panel.game.destroy', ['game'=>$item]) }}" class="btn btn-warning btn-xs"><span class="fa fa-trash-o"></span></a>
                                        @else
                                        <a href="{{ route('panel.game.destroy', ['game'=>$item]) }}" class="btn btn-danger btn-xs"><span class="fa fa-times"></span></a>
                                        @endif
                                   </div>
                              </td>
                         </tr>
